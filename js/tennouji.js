function recommendShop(preference, drinkType, foodType, moodType) {
  ...
  if (preference === '安くてうまい店がいい') {
    ...
    if (drinkType === '立ち飲み') {
      ...
      if (moodType === '並びたくない') {
        recommendation = 'スタンドふじ';
      } else if (moodType === '並んでもいい') {
        recommendation = 'すしセンター';
      }
    }
    //   お腹いっぱい食べたいの場合
...
function displayShopPhoto(recommendedShop) {
  ...
  // 店名に基づいて画像ファイル名を選択
  var photoFileNames = [];
  if (recommendedShop === 'マルツ') {
    photoFileNames = ['marutu.jpg', 'marutu_2.jpg'];
  } else if (recommendedShop === 'おか長') {
    photoFileNames = ['okanaga.jpg', 'okanaga_3.jpg'];
  } else if (recommendedShop === 'スタンドふじ') {
    photoFileNames = ['standfuji.jpg', 'standfuji_2.jpg'];
  } else if (recommendedShop === 'すしセンター') {
    photoFileNames = ['sushicenter.jpg', 'sushicenter_2.jpg'];
  }
...
$('#drinkTypeSelect').on('change', function () {
  ...
  // 選択した雰囲気を取得
  var moodType = $('#moodTypeSelect').val();
  // コンソールログでpreference, drinkType, moodTypeを確認 
  console.log('preference:', preference);
  console.log('drinkType:', drinkType);
  console.log('moodType:', moodType);

  // 希望が「綺麗な店がいい」かつ飲み方が「座り飲み」の場合、additionalOptions_2を表示する
  if (preference === '綺麗な店がいい' && drinkType === '座り飲み') {
    $('#additionalOptions_2').show();
  } else {
    $('#additionalOptions_2').hide();
  }

  // 希望が「お腹いっぱい食べたい」かつ飲み方が「座り飲み」の場合、additionalOptionsを表示する
  if (preference === 'お腹いっぱい食べたい' && drinkType === '座り飲み') {
    $('#additionalOptions').show();
  } else {
    $('#additionalOptions').hide();
  }

  // 希望が「お腹いっぱい食べたい」かつ飲み方が「座り飲み」の場合、ホルモンの選択肢を非表示にする
  if (preference === 'お腹いっぱい食べたい' && drinkType === '座り飲み') {
    $('#foodTypeSelect option[value="ホルモンが食べたい"]').hide();
  } else {
    $('#foodTypeSelect option[value="ホルモンが食べたい"]').show();
  }
});
